...
import com.karyar.mohsen.karyar.R.color
import com.karyar.mohsen.karyar.R.dimen
import com.karyar.mohsen.karyar.R.integer
import com.karyar.mohsen.karyar.login.BaseAuthController.Callback
...
class AuthPagerAdapter(private val host: Controller, private val viewPager: AnimatedViewPager,
    ...
    : RouterPagerAdapter(host), Callback {
    ...
    override fun getPageWidth(position: Int): Float {
        val textSize = host.resources!!.getDimension(
            dimen.folded_size)
        val textPadding = host.resources!!.getDimension(dimen.folded_label_padding)
    ...
    override fun show(nextController: BaseAuthController) {
        ...
        for (jIndex in 0 until pages.size()) {
            if (jIndex != controllerIndex) {
                pages.get(jIndex).fold()
            }
        }
    }
    ...
    private fun shiftSharedElements(pageOffsetX: Float, forward: Boolean) {
        ...
        val color = ContextCompat.getColor(context, if (forward) color.color_logo_sign_up else color.color_logo_log_in)
        ...
        shiftAnimator.duration = viewPager.resources.getInteger(integer.duration) / 2L
        shiftAnimator.start()
    }
